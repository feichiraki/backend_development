...
> 注意：
>
> ① 在C#中，每一条语句的是以英文分号`;`结尾。
>
> ② 代码和被使用类处于同一命名空间，则不用`using`。
...
嵌套命名空间的另外一种方法是使用`.`运算符(语义上是等价的)：

```c#
namespace Outer.Midle.Inner{
    // Class code
}
```
...
1. 完全限定名称：类似绝对目录，从全局空间的反斜线开始。
2. 非限定名称：省略空间名称，使用默认或当前的命名空间(所有命名空间由using引入，只需要通过具体名称来访问)。
3. 部分限定名称：在某些情况下，我们需要访问同级命名空间中的内容。



###### 4.4.1 完全限定名称

在C#中，还可以通过`完全限定名称`访问一个命名空间的成员。完全限定名称类似于我们电脑上文件路径的`绝对路径`。下面是完全限定命名的一个示例：
...
namespace nameSpaceDemo1
{
    class Program
    {
        static void Main(string[] args)
        {
            // 通过完全限定名称来访问 命名空间MyNameSpace>Student类>Show方法
...
在上述代码，我们是通过完全限定名称来访问Student类中Show方法的。细心的小伙伴会发现，我们是通过`.`运算符来访问Show方法的。

> 我们可以这么理解这个`.`运算符的含义：取其下级/成员的，类似于绝对路径中的`/`取下级目录或文件位置。

关于`.`，我们需要了解下面的两个术语：

* 在一条C#语句中，如果`.`后的成员是类、类的字段/属性等，我们称该行为为`访问`。
* 如果`.`后面的是一个方法，我们就称此操作为`调用`方法。



###### 4.4.2 非限定名称

为了避免开发人员每次都输入完全限定名称，C#中我们用`using`关键字来引入对应的命名空间，然后我们在编写代码时只需要用它们的类名来引用类即可。示例如下：
...
namespace nameSpaceDemo1
{
    class Program
    {
        static void Main(string[] args)
        {
            // 2.直接通过类名来引用命名空间下的类即可(简化为通过类名来访问类)
            Student.Show();
        }
    }
}
...
在同文件，命名空间同级常用到，不同文件类文件还是通过`using`来引入。
